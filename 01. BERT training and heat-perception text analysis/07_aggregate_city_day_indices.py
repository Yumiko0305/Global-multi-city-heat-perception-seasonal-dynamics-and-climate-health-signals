...
COUNTRY_HEMISPHERE = {
    # synthetic pool and common countries
...
def parse_args() -> Config:
    ...
    parser.add_argument("--total_posts_csv", default=None,
    ...
    parser.add_argument("--indicator_col", default="pred_label",
    ...
    parser.add_argument("--total_posts_col", default=None,
                        help="If input already contains a per-row city-day denominator column")

    parser.add_argument("--normalize_within_city_month", action="store_true",
                        help="Apply within-city-month median de-centering and 1/99 winsorization")
    parser.add_argument("--event_flag_col", default=None,
    ...
    parser.add_argument("--hppi_alpha", type=float, default=1.6,
                        help="Moderate admissible convexity parameter for ER-style HPPI")
    parser.add_argument("--high_hpii_min_level", type=int, default=5,
    ...
    parser.add_argument("--apply_eligibility_filters", action="store_true",
...
def read_input(cfg: Config) -> pd.DataFrame:
    ...
    if cfg.indicator_col not in df.columns:
        if "label" in df.columns:
            ...
            raise ValueError(f"Missing indicator column {cfg.indicator_col} and no label fallback found")
    ...
    df[cfg.indicator_col] = pd.to_numeric(df[cfg.indicator_col], errors="coerce").fillna(0).astype(int)
    ...
    if cfg.hemisphere_col not in df.columns:
        df[cfg.hemisphere_col] = [infer_hemisphere(c, k) for c, k in zip(df[cfg.country_col], df[cfg.continent_col])]
        ...
        df.loc[mask, cfg.hemisphere_col] = [infer_hemisphere(c, k) for c, k in zip(df.loc[mask, cfg.country_col], df.loc[mask, cfg.continent_col])]
...
def synthesize_dates(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    if cfg.month_col in out.columns:
        ...
        # fallback: spread rows over 12 months from configured start
        order = np.arange(len(out))
        out["__synthetic_month_num"] = ((cfg.synthetic_start_month - 1 + (order % 12)) % 12) + 1
        out["__synthetic_year"] = cfg.synthetic_start_year + ((cfg.synthetic_start_month - 1 + (order % 12)) // 12)

    out["__within_city_month_order"] = out.groupby([cfg.city_col, "__synthetic_year", "__synthetic_month_num"]).cumcount()
    # keep dates valid for all months; day cycles 1..28
...
def build_city_day_counts(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    meta_cols = [cfg.continent_col, cfg.country_col, cfg.hemisphere_col, "year", "month", "month_num", "season"]
    ...
    if cfg.total_posts_csv:
        ...
        denom_col = cfg.total_posts_col if cfg.total_posts_col and cfg.total_posts_col in total_df.columns else "total_posts"
        ...
        agg["denominator_source"] = np.where(agg["total_posts"].notna(), "external_city_day_totals", "observed_input_rows_proxy")
        agg["total_posts"] = agg["total_posts"].fillna(agg["observed_posts"])
    elif cfg.total_posts_col and cfg.total_posts_col in out.columns:
        temp = out.groupby(group_cols, dropna=False)[cfg.total_posts_col].first().reset_index().rename(columns={cfg.total_posts_col: "total_posts"})
        agg = agg.merge(temp, on=group_cols, how="left")
        agg["total_posts"] = agg["total_posts"].fillna(agg["observed_posts"])
        agg["denominator_source"] = np.where(agg["total_posts"].eq(agg["observed_posts"]), "observed_input_rows_proxy", "input_total_posts_col")
    else:
        agg["total_posts"] = agg["observed_posts"]
        agg["denominator_source"] = "observed_input_rows_proxy"

    agg["denominator_is_proxy"] = agg["denominator_source"].eq("observed_input_rows_proxy").astype(int)
    agg["daily_hpii_raw"] = np.where(agg["total_posts"] > 0, agg["heat_posts"] / agg["total_posts"], np.nan)
...
def normalize_city_month_series(city_day: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    if cfg.event_flag_col and cfg.event_flag_col in out.columns:
        out["__median_mask"] = (pd.to_numeric(out[cfg.event_flag_col], errors="coerce").fillna(0) == 0).astype(int)

    def _normalize_group(g: pd.DataFrame) -> pd.DataFrame:
        g = g.copy()
        base = g.loc[g["__median_mask"] == 1, "daily_hpii_raw"]
        if base.empty:
            median = g["daily_hpii_raw"].median()
        else:
            median = base.median()
    ...
    out = out.groupby([cfg.city_col, "month"], group_keys=False).apply(_normalize_group)
...
def apply_eligibility_filters(city_day: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, pd.DataFrame]:
    if not cfg.apply_eligibility_filters:
        city_summary = city_day.groupby(cfg.city_col).agg(
    ...
    total_days_per_city_season = city_day.groupby([cfg.city_col, "season"])["date"].nunique().reset_index(name="season_days")
    all_possible_days = city_day.groupby([cfg.city_col, "season"])["date"].nunique().reset_index(name="observed_unique_days")
    coverage = total_days_per_city_season.merge(all_possible_days, on=[cfg.city_col, "season"], how="left")
    coverage["coverage_frac"] = np.where(coverage["season_days"] > 0, coverage["observed_unique_days"] / coverage["season_days"], 0.0)

    city_summary = city_day.groupby(cfg.city_col).agg(
        total_heat_posts=("heat_posts", "sum"),
        active_days=("date", "nunique"),
    ).reset_index()
    min_cov = coverage.groupby(cfg.city_col)["coverage_frac"].min().reset_index(name="coverage_min_frac")
    city_summary = city_summary.merge(min_cov, on=cfg.city_col, how="left")
...
def compute_monthly_city_indices(city_day: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    monthly = city_day.groupby([cfg.city_col, cfg.continent_col, cfg.country_col, cfg.hemisphere_col, "year", "month", "month_num", "season"]).agg(
    ...
    monthly["month_hpii"] = np.where(monthly["month_total_posts"] > 0, monthly["month_heat_posts"] / monthly["month_total_posts"], np.nan)
...
def compute_hpvi(city_day: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    rows = []
    for keys, sub in base.groupby([cfg.city_col, cfg.continent_col, cfg.country_col, cfg.hemisphere_col, "season"]):
        ...
        if arr.empty:
            ...
            hpvi = 0.0 if abs(mean_val) < 1e-12 and abs(std_val) < 1e-12 else np.nan
            if abs(mean_val) >= 1e-12:
                hpvi = (std_val / abs(mean_val)) * 100.0
        rows.append({
            cfg.city_col: keys[0],
    ...
    hpvi_df["hpvi_high_binary"] = hpvi_df["hpvi"].apply(lambda x: 1 if pd.notna(x) and x > 30 else 0)
    return hpvi_df


def compute_city_season_indices(city_day: pd.DataFrame, monthly: pd.DataFrame, hpvi_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, List[float]]:
    seasonal = city_day.groupby([cfg.city_col, cfg.continent_col, cfg.country_col, cfg.hemisphere_col, "season"]).agg(
    ...
    peak_month = monthly.sort_values([cfg.city_col, "season", "month_hpii"], ascending=[True, True, False]).groupby([cfg.city_col, "season"]).head(1)
    peak_month = peak_month[[cfg.city_col, "season", "month", "month_hpii"]].rename(columns={
        "month": "peak_month_in_season",
        "month_hpii": "peak_month_hpii",
    })
    seasonal = seasonal.merge(peak_month, on=[cfg.city_col, "season"], how="left")
    seasonal = seasonal.merge(hpvi_df, on=[cfg.city_col, cfg.continent_col, cfg.country_col, cfg.hemisphere_col, "season"], how="left")

    levels, breaks = assign_jenks_levels(seasonal["seasonal_hpii"], n_classes=7)
    seasonal["hpii_level"] = levels.astype("Int64")
    seasonal["hpii_level_roman"] = seasonal["hpii_level"].map(ROMAN)
    seasonal["high_hpii_binary"] = seasonal["hpii_level"].apply(lambda x: 1 if pd.notna(x) and int(x) >= cfg.high_hpii_min_level else 0)
    ...
    def typology(row: pd.Series) -> str:
        ...
        if (not hi) and (not hv):
            return "Stable Low"
        return "Low Intensity / High Variability"
...
def compute_city_typology_summary(seasonal: pd.DataFrame, monthly: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    if seasonal.empty:
        return pd.DataFrame(columns=[cfg.city_col, "peak_season", "total_dominant_season", "peak_total_coincide", "multi_season_high_hpii", "counter_seasonal_label"])

    peak_any = monthly.sort_values([cfg.city_col, "month_hpii"], ascending=[True, False]).groupby(cfg.city_col).head(1)
    peak_any = peak_any[[cfg.city_col, cfg.continent_col, cfg.country_col, cfg.hemisphere_col, "season", "month", "month_hpii"]].rename(columns={
        "season": "peak_season",
        "month": "peak_month",
        "month_hpii": "peak_month_hpii_global",
    })

    total_dom = seasonal.sort_values([cfg.city_col, "seasonal_hpii_daily_sum"], ascending=[True, False]).groupby(cfg.city_col).head(1)
    total_dom = total_dom[[cfg.city_col, "season", "seasonal_hpii_daily_sum", "seasonal_hpii"]].rename(columns={
    ...
    high_counts = seasonal.groupby(cfg.city_col)["high_hpii_binary"].sum().reset_index(name="n_high_hpii_seasons")
    high_counts["multi_season_high_hpii"] = (high_counts["n_high_hpii_seasons"] >= 2).astype(int)
    ...
    peak_lookup = peak_any[[cfg.city_col, "peak_season", "peak_month_hpii_global"]].copy()

    summer_total = season_lookup[season_lookup["season"] == "Summer"][[cfg.city_col, "seasonal_hpii_daily_sum"]].rename(columns={"seasonal_hpii_daily_sum": "summer_total_hpii_sum"})
    winter_total = season_lookup[season_lookup["season"] == "Winter"][[cfg.city_col, "seasonal_hpii_daily_sum"]].rename(columns={"seasonal_hpii_daily_sum": "winter_total_hpii_sum"})
    summer_peak = seasonal[seasonal["season"] == "Summer"][[cfg.city_col, "peak_month_hpii"]].rename(columns={"peak_month_hpii": "summer_peak_month_hpii"})
    winter_peak = seasonal[seasonal["season"] == "Winter"][[cfg.city_col, "peak_month_hpii"]].rename(columns={"peak_month_hpii": "winter_peak_month_hpii"})
    ...
    peak_cs = (out["winter_peak_month_hpii"].fillna(-np.inf) > out["summer_peak_month_hpii"].fillna(-np.inf))
    total_cs = (out["winter_total_hpii_sum"].fillna(-np.inf) > out["summer_total_hpii_sum"].fillna(-np.inf))
    ...
    out["counter_seasonal_binary"] = (out["counter_seasonal_label"] != "NoCounterSeasonal").astype(int)
    return out.sort_values(cfg.city_col).reset_index(drop=True)


def volume_strata(city_day: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    annual = city_day.groupby(cfg.city_col).agg(
...
def _scaled_signal(series: pd.Series, kind: str) -> pd.DataFrame:
    ...
    tier_weight = tiers.map({"low": 1.0, "medium": 2.0, "high": 3.0}).astype(float).fillna(1.0)
...
def compute_hppi(seasonal: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    for (continent, season), sub in seasonal.groupby([cfg.continent_col, "season"]):
        hppi_hpii = normalized_er_polarization(sub["seasonal_hpii"], alpha=cfg.hppi_alpha, kind="hpii")
...
def write_outputs(
    ...
    summary = {
        ...
        "n_continents": int(city_day[cfg.continent_col].nunique()) if cfg.continent_col in city_day.columns and not city_day.empty else 0,
        "used_indicator_col": cfg.indicator_col,
        "denominator_proxy_fraction": float(city_day["denominator_is_proxy"].mean()) if not city_day.empty else 0.0,
        "synthetic_date_fraction": float(city_day["date_is_synthetic"].mean()) if not city_day.empty else 0.0,
        "hemisphere_inferred_fraction": float(city_day["hemisphere_is_inferred"].mean()) if not city_day.empty else 0.0,
        "normalization_applied": bool(cfg.normalize_within_city_month),
        "hpii_jenks_breaks": jenks_breaks,
...
def main() -> None:
    ...
    seasonal, jenks_breaks = compute_city_season_indices(city_day, monthly, hpvi_df, cfg)
    ...
    # merge tweet-volume strata into city-season summary for convenience
    seasonal = seasonal.merge(volume[[cfg.city_col, "tweet_volume_stratum"]], on=cfg.city_col, how="left")
    city_typology = city_typology.merge(volume[[cfg.city_col, "tweet_volume_stratum"]], on=cfg.city_col, how="left")

    paths = write_outputs(city_day, monthly, seasonal, city_typology, hppi, volume, eligibility, jenks_breaks, cfg)
