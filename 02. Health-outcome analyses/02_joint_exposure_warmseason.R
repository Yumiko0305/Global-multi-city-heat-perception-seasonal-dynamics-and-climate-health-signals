## =============================================================================
## Script 02: Joint temperature-perception exposure effects — warm season (primary)
##
## Defines 4 joint exposure categories:
##   Category 1: Normal temperature & Low perception  [reference]
##   Category 2: Normal temperature & High perception (HPII > 0)
##   Category 3: Extreme heat (>=95th pctile) & Low perception
##   Category 4: Extreme heat (>=95th pctile) & High perception
##
## DLNM specification:
##   crossbasis: strata argvar (breaks = c(2,3,4)), strata arglag, lag = 2
##   family: quasi-Poisson
##   covariates: factor(dow) + holiday + factor(month) + pm25
##
## Loop 1: city-level RR (crosspred)
## Loop 2: crossreduce -> coef + vcov -> mixmeta pooled estimates
##
## Input:  final_dat.rds
## Output: effect_size/hospitalization.csv
##         effect_size/hospitalization.pdf
##         effect_size/death.csv
##         effect_size/death.pdf
## =============================================================================
...
dir.create('effect_size', showWarnings = FALSE)
ggsave(p, filename = 'effect_size/hospitalization.pdf', width = 10, height = 12)
write_csv(plot_dat, file = 'effect_size/hospitalization.csv')
...
p
dir.create('effect_size', showWarnings = FALSE)
ggsave(p, filename = 'effect_size/death.pdf', width = 10, height = 12)
write_csv(plot_dat, file = 'effect_size/death.csv')
